...
export function HomePage({ recentProjects, recentConversations, onNavigate, onProjectClick, onConversationClick, onNewChat }: HomePageProps) {
          ...
          <Card className="border-border/30 shadow-lg bg-card/90 backdrop-blur-sm">
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <Button variant="ghost" size="sm" onClick={onNewChat} className="text-primary hover:text-primary hover:bg-primary/20 rounded-xl">
                  New Chat
                </Button>
              </div>
            </CardHeader>
              ...
              {recentConversations.length === 0 ? (
                ...
                <div className="space-y-4">
                  {recentConversations.slice(0, 3).map((conversation) => {
                    ...
                    const hasPattern = conversation.messages.some(m => m.isPattern);

                    return (
                      <div
                        key={conversation.id}
                        className="flex items-start gap-3 p-4 bg-gradient-to-r from-muted/30 to-muted/10 rounded-2xl hover:from-muted/50 hover:to-muted/20 transition-all duration-200 cursor-pointer border border-border/30"
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <h4 className="font-medium text-sm line-clamp-1 mb-1">{conversation.title}</h4>
                          {lastMessage && (
